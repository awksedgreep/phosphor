...
> Follow the current, tested keystrokes: [Build a CRM from an empty database](docs/BUILD-A-CRM.md). Start with `./target/release/phosphor crm.db` to keep your work. A no-argument scratch session is temporary; F9 Save Database keeps it in a new file.

## Watch it work

These GIFs are recorded demonstrations generated by `tools/demo/demo.sh`.
They may show an earlier version; the CRM guide and terminal regressions
describe the current keystrokes.
...
in-app F1 help — `phosphor --manual`), the step-by-step
